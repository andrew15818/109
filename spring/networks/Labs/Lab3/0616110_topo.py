...
def runNAT(routers):
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.0/26 -o eth0 -j SNAT --to-source 140.113.0.30')
    routers['r1'].cmd('iptables -t nat -A POSTROUTING -s 192.168.1.64/26 -o eth0 -j SNAT --to-source 140.113.0.40')
    routers['r1'].cmd('iptables -t nat -A PREROUTING -p tcp -s 140.113.0.40 --dport 80 -j DNAT --to-destination 192.168.1.66:8000')
...
def runBGP(routers):
    ...
    print('[+] Run the BGP config files')
